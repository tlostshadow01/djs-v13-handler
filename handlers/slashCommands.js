				...
				if(cmdSetup && cmdSetup.Folder) {
					...
					for (let file of slashCommands) {
						...
						if (pull.name && pull.description) {
								...
								if(pull.options && pull.options.length > 0){
									for(const option of pull.options){
										if(option.User && option.User.name && option.User.description){
											...
											console.log(`Bir Seçenek'in Adı ve/ve Açıklaması eksik ${pull.name}`)
										}
									}
								}
								return subcommand;
							...
							console.log(file, `error -> help.name eksik veya help.name bir dize değil.`.brightRed);
							continue;
						}
					}
					...
					return console.log(`${dir} Alt Komut Klasörü, dirSetup Yapılandırmasında değil!`);
				}
			} else {
						...
						if(pull.options && pull.options.length > 0){
							for(const option of pull.options){
								if(option.User && option.User.name && option.User.description){
									...
									console.log(`Bir Seçenek, ${pull.name} öğesinin Adı ve/veya Açıklaması eksik`)
								}
							}
						}
			...
			if(config.loadSlashsGlobal){
				...
				client.guilds.cache.map(g => g).forEach((guild) => {
					try{
						...
						.then(slashCommandsData => {
							console.log(`${slashCommandsData.size} (/) Komutu ${`(toplam ${slashCommandsData.map(d => d.options).flat().length} komut)`.green} ${`${guild.name} isimli sunucuda yüklendi.`.underline}`.brightGreen); 
						}).catch((e)=>console.log(e));
					}catch (e){
		...
		client.on("guildCreate", (guild) => {
			try{
				if(!config.loadSlashsGlobal){
					guild.commands.set(allCommands)
						.then(slashCommandsData => {
							console.log(`${slashCommandsData.size} slashCommands ${`(With ${slashCommandsData.map(d => d.options).flat().length} Subcommands)`.green} Yüklendi. ${`${guild.name}`.underline}`.brightGreen); 
						}).catch((e)=>console.log(e));
				}
			}catch (e){
				console.log(String(e).grey)
